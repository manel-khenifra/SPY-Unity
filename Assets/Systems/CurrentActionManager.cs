...
public class CurrentActionManager : FSystem
{
	...
	public bool whileValid(WhileAction whileAction, GameObject scripted)
	{
		...
		// get absolute target position depending on player orientation and relative direction to observe
		Vector2 vec = new Vector2();
		switch (getDirection(scripted.GetComponent<Direction>().direction, whileAction.whileDirection))
		{
			case Direction.Dir.North:
		...
		// check target position
		switch (whileAction.whileEntityType)
		{
			case 0: // walls
				foreach (GameObject go in wallGO)
					if (go.GetComponent<Position>().x == scripted.GetComponent<Position>().x + vec.x &&
					 go.GetComponent<Position>().z == scripted.GetComponent<Position>().z + vec.y)
						whileok = !whileAction.whileNot;
				break;
			case 1: // doors
				foreach (GameObject go in doorGO)
					if (go.GetComponent<Position>().x == scripted.GetComponent<Position>().x + vec.x &&
	...
	public GameObject getNextAction(GameObject currentAction, GameObject agent){
		...
		Debug.Log("TEST GET NEXT ACTION");
		if (current_ba != null)
		{
		...
		else if (currentAction.GetComponent<WhileAction>())
		{
			...
			while (!whileValid(whileAction, agent))
			{
				if (whileAction.firstChild != null)
				{
					Debug.Log("ACTION WHILE 1");
					// return first action
				...
				/*else
				{
					Debug.Log("ACTION WHILE 2");
					Debug.Log(whileAction.next);
					Debug.Log(whileAction.next.ToString());
					// return next action
